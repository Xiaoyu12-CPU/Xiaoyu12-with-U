...
当前实现进度：Phase 1、Phase 1.5、Phase 2-A 至 Phase 2-E、Phase 3-A 至 Phase 3-G、Phase 4 提醒系统和 Phase 5 输入感知均已完成；CPU、Memory、Network、Storage、Battery 使用真实采样。v0.4.4 已完成四窗口桌面重建后的 UI 与稳定性收尾，v0.4.5 已完成中英日界面、品牌信息、默认布局固化和桌宠位置持久化，v0.4.6 正在增加控制中心外观主题、背景模糊与文字阴影。Phase 6 自定义皮肤包与 Phase 7 AI 互动仍属于后续规划。
...
- 在背景设置上方提供可横向拖动的主题卡片选择器。
- 内置 v0.4.5 默认外观、开发机当前确认的蜜柑主题，以及无背景图片的空白主题起点。
- 主题预设保存完整 `controlCenter` 外观快照，切换不改变其他应用设置。
- 背景图片增加独立模糊度；内容文字增加颜色、透明度、大小与模糊范围可调的环绕阴影。

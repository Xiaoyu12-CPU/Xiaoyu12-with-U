...
> 当前版本：Windows v0.5.1；macOS 安装包继续使用 v0.5.0。软件仍在开发期，功能与外观都可能变化，欢迎试用反馈。

## v0.5.1 更新（Windows x64）

- **点击开心状态修复**：Windows 原生窗口拖动可能吞掉普通点击的 `click` 事件；现在没有发生拖动时，松开鼠标会补触发一次 `happy`，迟到的点击事件不会重复触发。
- **保留正常交互**：连续点击仍可正常响应，真实拖拽和取消操作不会误触发开心；macOS 原有处理路径保持不变。
- **最小修复范围**：沿用 v0.5.0 的 A01–A11 美术资源、动画和设置，仅提供新的 Windows x64 安装程序；macOS 用户继续使用 [v0.5.0](https://github.com/Xiaoyu12-CPU/Xiaoyu12-with-U/releases/tag/v0.5.0)。
- 新增 6 组点击与拖拽回归场景；Windows 实机验证尚未完成。

## v0.5.0 更新

- **完整表情资源 A01–A11**：加入慌张、注意、开心和使用银白笔记本工作的表情及动作差分；工作动画中人物轻微下移，笔记本与小熊标志保持固定。
- **统一透明像素资源**：正式表情全部使用 200×200 RGBA PNG，清理默认、过渡和闭眼笑的发丝白边，并优化图片体积。
- **动画编辑器尺寸修复**：编辑器重新读取图片实际尺寸，避免沿用旧配置里的 1254×1254；读取尺寸不会产生未保存修改。
- **包含主题开发成果**：整合 v0.4.6 / v0.4.6.1 的默认、蜜柑和自定义主题，支持各主题独立编辑、背景模糊、文字阴影及正确的背景切换。
- **保留已有自定义设置**：新内置动画用于新安装或缺少用户覆盖的状态；已保存的自定义动画、设置和窗口布局继续优先，本轮不新增迁移。

| 状态 | 内置画面顺序 | 每帧时长与播放方式 |
| --- | --- | --- |
| idle | A01 → A02 → A03 → A01 | 200 / 100 / 200 / 0 ms；末帧保持，随机等待 10 / 3 / 30 / 22 秒后重播 |
| happy | A08 → A09 → A08 | 120 / 120 / 160 ms，连续循环 |
| tired、dragging | A04 → A05 → A04 | 120 / 120 / 160 ms，连续循环 |
| alert | A06 → A07 → A06 | 120 / 120 / 160 ms，连续循环 |
| working | A10 → A11 → A10 | 120 / 120 / 160 ms，连续循环 |
| sleep | 原占位画面 | 保持原配置 |

## v0.4.6.1 历史开发基线（已纳入 v0.5.0）

- **主题成为可编辑实体**：默认主题、蜜柑主题和每个自定义主题分别保存自己的完整控制中心外观；修改当前主题后，切换回来仍会恢复该主题的修改。
- **空白主题行为修正**：每次点击“新增主题”都会创建并立即切换到新的“自定义主题 N”，从基础默认参数开始且不继承当前背景图片。
- **背景与主题一致**：背景图片、颜色、透明度、模糊、文字阴影和现有卡片/侧栏参数均归属于各自主题，切换主题时不会再显示上一个主题的背景。
- **兼容 v0.4.6 设置**：旧版全局控制中心外观会迁移到对应主题，保留已有背景、颜色、透明度与文字阴影设置。

## v0.4.6 历史开发基线（已纳入 v0.5.0）

- **控制中心外观主题**：在背景设置上方增加可横向拖动的主题选择器，提供原默认主题、蜜柑主题和用于重新设计的空白主题。
- **蜜柑主题内置化**：将开发机当前确认并优化为 1280×720 的背景图以及配色、透明度、文字阴影和卡片参数保存为内置“蜜柑主题”，换电脑或重新安装后仍可直接选择。
- **背景图片模糊度**：增加 `0–30 px` 可调范围，模糊只作用于背景图片层。
- **内容文字阴影**：增加阴影颜色、透明度、大小和模糊范围，使文字在以图片为主体的背景上仍保持清晰。

## v0.4.5.2 更新（Windows x64）

- **拖动状态恢复**：Windows 原生窗口拖动吞掉松开事件时，应用会通过主鼠标按键状态兜底结束拖动会话，桌宠不再永久卡在 `dragging / 拖动中`。
- 本次仅发布新的 Windows x64 安装程序；macOS 用户继续使用 v0.4.5.1。

## v0.4.5.1 更新

- **鼠标窗口拖动优化**：移除鼠标图案上方的三段虚线拖动提示，将拖动区域迁移到鼠标背部，使操作位置与图案本身保持一致。

## v0.4.5 更新

- **三语界面**：控制中心左下角新增中文、English、日本語切换，选择会自动保存；控制中心与桌面浮层的固定界面文字同步切换。
- **品牌信息更新**：控制中心左上角改用正式应用图标，副标题显示 `v0.4.5`。
- **确认当前默认外观**：桌宠缩放、三个浮层的尺寸、配色、透明度和相对位置，以及控制中心主题，固定为新安装和“恢复默认”时使用的 v0.4.5 默认值。
- **完整窗口位置记忆**：除三个浮层外，桌宠本体的位置现在也会保存；下次打开时先恢复桌宠，再恢复浮层相对布局。
- **右键菜单完整显示**：菜单靠近桌宠窗口边缘时会自动调整位置，不再被窗口边界裁切。
- **首次安装仍保持简洁**：系统状态、键盘历史、鼠标可视化和对应监听默认关闭，只显示桌宠本体。
...
美术资源及 `A01 默认 / A02 过渡 / A03 闭眼笑 / A04 慌张 / A05 慌张下移差分 / A06 注意 / A07 注意下移差分 / A08 开心 / A09 开心下移差分 / A10 工作中 / A11 工作中下移差分` 标记见 [ART_ASSETS.md](./ART_ASSETS.md)，后续表情制作可直接引用该索引。

生成、局部修正、白边清理、差分动画和状态替换遵循 [ART_WORKFLOW.md](./ART_WORKFLOW.md)。
...
| v0.4.5 | 增加中英日语言切换，更新控制中心品牌信息，固化当前外观和四窗口位置记忆，并修复右键菜单裁切 |
| v0.4.5.1 | 移除鼠标可视化上方的虚线拖动提示，将拖动区域迁移到鼠标背部 |
| v0.4.5.2 | Windows x64 专项修复：原生拖动丢失松开事件后不再卡在拖动状态 |
| v0.4.6（历史开发基线） | 增加横向外观主题选择器、蜜柑/默认/空白主题，以及背景模糊与内容文字阴影设置；已纳入 v0.5.0 |
| v0.4.6.1（历史开发基线） | 将主题修正为独立可编辑实体，修复主题修改、空白新建、背景切换和旧设置迁移；已纳入 v0.5.0 |
| v0.5.0 | 整合主题功能与 A01–A11 表情资源，统一 200×200 透明图片、清理白边、修复编辑器尺寸，并更新各状态内置动画 |
| v0.5.1 | Windows x64 专项修复：恢复普通点击的 happy 状态，过滤迟到的重复点击，沿用 v0.5.0 资源和动画 |

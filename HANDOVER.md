...
`main` 与 `develop` 当前仍停留在 `v0.4.5.1`；`v0.4.5.2` 是单独发布的 Windows x64 拖动状态修复。`codex/v0.4.6` 已包含该修复以及之后的控制中心外观开发。旧发布标签只作为历史快照保留，不应复用或移动。
...
3. 验收完成后创建新的不可移动标签 `v0.4.6`，不要复用或移动已经公开的旧标签。

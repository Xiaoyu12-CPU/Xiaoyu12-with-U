  ...
  <div class="settings-sections" data-settings-category="general">
    <article>
      ...
      <label class="setting-row scale-row">
        <span><strong>{{ $t("桌宠大小") }}</strong><small>50% ～ 200%</small></span>
      ...
      <label class="setting-row">
        <span><strong>{{ $t("始终置顶") }}</strong><small>{{ $t("控制桌宠及三个浮层窗口是否始终置顶。") }}</small></span>
        <input class="toggle" type="checkbox" :checked="settings.appearance.alwaysOnTop" @change="updateAlwaysOnTop" />
      </label>
      <label class="setting-row">
        <span><strong>{{ $t("浮层跟随桌宠") }}</strong><small>{{ $t("系统状态、键盘历史和鼠标可视化保持各自相对位置。") }}</small></span>
    ...
    <article>
      <div class="section-heading">
        <h3>{{ $t("动画") }}</h3>
        <p>{{ $t("关闭后保持当前有效帧，重新开启后继续播放。") }}</p>
      </div>
      <label class="setting-row">
        <span><strong>{{ $t("启用动画") }}</strong></span>

...
默认背景以 Vite 正式 Asset `src/assets/control-center/default-background.jpg` 随前端 Bundle 发布；蜜柑背景使用优化后的 1280×720 图片 `src/assets/control-center/mikan-background.png`，主题卡使用独立缩略图避免为预览重复解码完整图片。两张图分别通过 `builtin:shipping-default` 与 `builtin:mikan` token 和用户 Managed filename 区分。Resolver 遇到 builtin token 直接使用跨平台 bundled URL，不访问 app data；Managed filename 继续由 Rust command 从 `app_data_dir()/control-center/background/` 读取。Reset Appearance 清理当前 Managed 副本后恢复 Shipping Theme 与 builtin token，但绝不删除项目内置 Asset。
